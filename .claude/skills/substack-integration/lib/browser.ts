...
export async function readInput<T>(): Promise<T> {
  return new Promise((resolve, reject) => {
    ...
    process.stdin.on('end', () => {
      try { resolve(JSON.parse(data)); } catch (err) { reject(new Error(`Invalid JSON input: ${err}`)); }
    });
...
export async function getBrowserContext(): Promise<BrowserContext> {
  if (!fs.existsSync(config.authPath)) {
    throw new Error('Substack browser authentication not configured. Run substack-browser-setup first.');
  }

  cleanupLockFiles();

  return await chromium.launchPersistentContext(config.browserDataDir, {
